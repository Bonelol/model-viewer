...
export class ModelViewDetailsComponent implements OnInit, OnChanges {
    ...
    constructor(
        private readonly service: ModelDescribeService,
        private readonly store: ModelViewStoreService
    ) { }
    ...
    async onViewProperty(property: ModelClassDetails) {
        ...
        const typeDetails = await this.service
            .getModelDesctiption(property.TypeName)
            .toPromise();
        const descriptor = {
            type: property.IsCollection
                ? ModelViewOptions.List
                : ModelViewOptions.Details,
        ...
        const count = index + 1;
        this.store.descriptors = this.store.descriptors
            .slice(0, count)
            .concat(descriptor);
    }
